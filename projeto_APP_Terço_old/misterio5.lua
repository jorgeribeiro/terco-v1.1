...
function scene:create( event )
	...
	print( "Tela 5 Misterio: event created" )
end
...
function scene:show( event )
	...
	if  phase == "did"  then
		print( "Tela 5 Misterio: show event, phase did" )
		previousScene = composer.getSceneName( "previous" )
		composer.removeScene( "previousScene" )
		print( "Tela 5 Misterio: Previous Scene: destroy" )
		local background = display.newImage("misterio5.png", display.contentCenterX, display.contentCenterY)

		local buttonReturn = widget.newButton
		{
			id = "buttonReturn",
			defaultFile = "buttoninicio.png",
			overFile = "buttoninicioover.png",
			onRelease = function() composer.gotoScene( "tela1" )
			end
		}
		buttonReturn.x = display.contentCenterX + 110
		buttonReturn.y = display.contentCenterY + 210
		...
		local buttonDeusSanto = widget.newButton
		{
			id = "buttonDeusSanto",
			defaultFile = "deussanto.png",
			onRelease = function() composer.gotoScene( "teladeussanto" )				
			end
		}
		buttonDeusSanto.x = display.contentCenterX
		buttonDeusSanto.y = display.contentCenterY + 180
		...
		local arrow1 = widget.newButton
		{
			...
			onRelease = function() composer.gotoScene( "misterio4" )
			end
		}
...
function scene:hide( event )
	local phase = event.phase
	if phase == "will" then
		print( "Tela 5 Misterio: hide event, phase will" )
	end
end

--Destroy the scene
function scene:destroy( event )
	print( "((destroying Tela 5 Misterio view))" )
end
