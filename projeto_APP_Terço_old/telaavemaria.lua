...
function scene:create( event )
	...
	print( "Tela Ave Maria: event created" )
end
...
function scene:show( event )
	...
	if  phase == "did"  then
		print( "Tela Ave Maria: show event, phase did" )
		previousScene = composer.getSceneName( "previous" )
		composer.removeScene( "tela2" )
		print( "Tela 2: destroy" )
		local background = display.newImage("telaavemaria.png", display.contentCenterX, display.contentCenterY)
...
function scene:hide( event )
	local phase = event.phase
	if phase == "will" then
		print( "Tela Ave Maria: hide event, phase will" )
	end
end

--Destroy the scene
function scene:destroy( event )
	print( "((destroying Tela Ave Maria's view))" )
end
