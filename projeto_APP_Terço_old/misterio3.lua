...
function scene:create( event )
	...
	print( "Tela 3 Misterio: event created" )
end
...
function scene:show( event )
	...
	if  phase == "did"  then
		print( "Tela 3 Misterio: show event, phase did" )
		previousScene = composer.getSceneName( "previous" )
		composer.removeScene( "previousScene" )
		print( "Tela 3 Misterio: Previous Scene: destroy" )
		local background = display.newImage("misterio3.png", display.contentCenterX, display.contentCenterY)

		local button4Misterio = widget.newButton
		{
			id = "button4Misterio",
			defaultFile = "button4misterio.png",
			overFile = "button4misterioover.png",
			onRelease = function() composer.gotoScene( "misterio4" )
			end
		}
		button4Misterio.x = display.contentCenterX + 110
		button4Misterio.y = display.contentCenterY + 210
...
function scene:hide( event )
	local phase = event.phase
	if phase == "will" then
		print( "Tela 3 Misterio: hide event, phase will" )
	end
end

--Destroy the scene
function scene:destroy( event )
	print( "((destroying Tela 3 Misterio view))" )
end
